...
- [x] T003 [P] Create `JsonbToStringReadingConverter` and add custom `@Query` INSERT with `CAST(? AS jsonb)` to `AuditLogRepository` for JSONB writing (global writing converters rejected — they break all VARCHAR fields). In `backend/src/main/java/com/worklog/infrastructure/persistence/`
...
- [x] T005 [P] Create `InetToStringReadingConverter` and add `CAST(? AS inet)` to `AuditLogRepository` custom `@Query` INSERT for INET writing (global writing converters rejected — they break all VARCHAR fields). In `backend/src/main/java/com/worklog/infrastructure/persistence/`
...
- [x] T007 Register 2 reading converters in `jdbcCustomConversions()` bean alongside existing UserId/RoleId converters in `backend/src/main/java/com/worklog/infrastructure/persistence/PersistenceConfig.java`

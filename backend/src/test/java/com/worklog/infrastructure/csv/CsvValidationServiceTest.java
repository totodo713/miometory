...
class CsvValidationServiceTest {
    ...
    void testMultipleErrors() {
        var result = validationService.validateRow(1, "invalid-date", "", "abc", "A".repeat(501));
    ...
    @CsvSource({"2025-12-31, true", "2026-01-01, true", "2026-12-31, true", "2027-01-01, true"})
    void testDateBoundaries(String date, boolean expectedValid) {
        ...
        assertEquals(
                expectedValid,
                result.isValid()
                        || !result.getErrors().stream().anyMatch(e -> e.contains("Date cannot be in the future")));
    }

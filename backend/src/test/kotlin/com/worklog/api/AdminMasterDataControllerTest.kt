...
class AdminMasterDataControllerTest : AdminIntegrationTestBase() {
    ...
    inner class FiscalYearPresets {
        ...
        fun `list returns 200 with seeded data for system admin`() {
            mockMvc.perform(get("$basePath/fiscal-year-rules").with(user(adminEmail)))
        ...
        fun `list returns 403 for tenant admin`() {
            mockMvc.perform(get("$basePath/fiscal-year-rules").with(user(tenantAdminEmail)))
        ...
        fun `create returns 201`() {
            ...
            mockMvc.perform(
                post("$basePath/fiscal-year-rules")
        ...
        fun `create duplicate name returns 409`() {
            ...
            mockMvc.perform(
                post("$basePath/fiscal-year-rules").with(user(adminEmail))
                    .contentType(MediaType.APPLICATION_JSON)
        ...
        fun `update returns 200`() {
            ...
            mockMvc.perform(
                put("$basePath/fiscal-year-rules/$id").with(user(adminEmail))
                    .contentType(MediaType.APPLICATION_JSON)
        ...
        fun `deactivate and activate returns 200`() {
            ...
            mockMvc.perform(patch("$basePath/fiscal-year-rules/$id/deactivate").with(user(adminEmail)))
                .andExpect(status().isOk)
            mockMvc.perform(patch("$basePath/fiscal-year-rules/$id/activate").with(user(adminEmail)))
                .andExpect(status().isOk)
        }
        ...
        fun `search filters by name`() {
            ...
            mockMvc.perform(
                get("$basePath/fiscal-year-rules").with(user(adminEmail)).param("search", unique),
            ).andExpect(status().isOk)
    ...
    inner class MonthlyPeriodPresets {
        ...
        fun `list returns 200 with seeded data`() {
            mockMvc.perform(get("$basePath/monthly-period-rules").with(user(adminEmail)))
                .andExpect(status().isOk)
                .andExpect(jsonPath("$.content").isArray)
        ...
        fun `create returns 201`() {
            ...
            mockMvc.perform(
                post("$basePath/monthly-period-rules").with(user(adminEmail))
                    .contentType(MediaType.APPLICATION_JSON)
        ...
        @Test
        fun `list returns 403 for tenant admin`() {
            mockMvc.perform(get("$basePath/monthly-period-rules").with(user(tenantAdminEmail)))
                .andExpect(status().isForbidden)
        }
    }
    ...
    inner class HolidayCalendars {
        ...
        fun `create calendar and add entry returns 201`() {
            ...
            // Verify entries
            mockMvc.perform(get("$basePath/holiday-calendars/$calId/rules").with(user(adminEmail)))
                .andExpect(status().isOk)
        ...
        fun `list seeded Japan entries`() {
            ...
            mockMvc.perform(
                get("$basePath/holiday-calendars/00000000-0000-0000-0000-bc0000000001/rules")
                    .with(user(adminEmail)),
            ).andExpect(status().isOk)
        ...
        fun `delete entry returns 200`() {
            ...
            mockMvc.perform(
                delete("$basePath/holiday-calendars/$calId/rules/$entryId").with(user(adminEmail)),
            ).andExpect(status().isOk)
        }

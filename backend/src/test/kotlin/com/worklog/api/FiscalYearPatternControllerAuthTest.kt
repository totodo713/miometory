...
class FiscalYearPatternControllerAuthTest : AdminIntegrationTestBase() {
    ...
    fun setup() {
        ...
        systemAdminEmail = "sysadmin-fy-$suffix@test.com"
        tenantAdminEmail = "tadmin-fy-$suffix@test.com"
        regularEmail = "user-fy-$suffix@test.com"
    ...
    fun `POST returns 201 for tenant admin creating in own tenant`() {
        mockMvc.perform(
            post("/api/v1/tenants/$ADM_TEST_TENANT_ID/fiscal-year-patterns")
                ...
                .content(
                    """{"name":"Auth Test FY ${UUID.randomUUID().toString().take(4)}","startMonth":4,"startDay":1}""",
                ),
        )

...
public class ApprovalService {
    ...
    public MonthlyApprovalId submitMonth(SubmitMonthForApprovalCommand command) {
        ...
        Set<WorkLogEntryId> workLogIds = workLogEntryIds.stream()
            .map(WorkLogEntryId::of)
            .collect(Collectors.toSet());
        
        Set<AbsenceId> absIds = absenceIds.stream()
            .map(AbsenceId::of)
            .collect(Collectors.toSet());
        ...
        for (UUID entryId : workLogEntryIds) {
            WorkLogEntry entry = workLogRepository.findById(WorkLogEntryId.of(entryId))
                .orElseThrow(() -> new DomainException(
                    "WORK_LOG_ENTRY_NOT_FOUND",
                    "Work log entry not found: " + entryId
                ));
        ...
        for (UUID absenceId : absenceIds) {
            Absence absence = absenceRepository.findById(AbsenceId.of(absenceId))
                .orElseThrow(() -> new DomainException(
                    "ABSENCE_NOT_FOUND",
    ...
    public void approveMonth(ApproveMonthCommand command) {
        ...
        MonthlyApproval approval = approvalRepository.findById(command.approvalId())
            .orElseThrow(() -> new DomainException(
                "APPROVAL_NOT_FOUND",
                "Monthly approval not found: " + command.approvalId()
            ));

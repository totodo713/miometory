...
public class DateInfoService {
    ...
    public DateInfo getDateInfo(UUID organizationId, LocalDate date) {
        // Load organization
        Organization org = organizationRepository.findById(new OrganizationId(organizationId))
        ...
        if (fiscalYearPatternId == null) {
            throw new IllegalStateException(
                "No fiscal year pattern found for organization: " + organizationId
            );
        }

        if (monthlyPeriodPatternId == null) {
            throw new IllegalStateException(
                "No monthly period pattern found for organization: " + organizationId
            );
        }
    ...
    private UUID resolveFiscalYearPattern(Organization org) {
        ...
        while (current != null) {
            ...
            if (current.getParentId() != null) {
                current = organizationRepository.findById(current.getParentId())
                    .orElse(null);
            } else {

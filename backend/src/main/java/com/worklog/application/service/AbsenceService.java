...
public class AbsenceService {
    ...
    public AbsenceService(JdbcAbsenceRepository absenceRepository, JdbcWorkLogRepository workLogRepository) {
    ...
    public UUID recordAbsence(CreateAbsenceCommand command) {
        ...
        validateCombinedDailyLimit(
                command.memberId(), command.date(), hours, null // no absence to exclude for creates
                );
    ...
    public void updateAbsence(UpdateAbsenceCommand command) {
        Absence absence = absenceRepository
                .findById(AbsenceId.of(command.id()))
                .orElseThrow(() -> new DomainException("ABSENCE_NOT_FOUND", "Absence not found: " + command.id()));

        // Check version for optimistic locking
        if (absence.getVersion() != command.version()) {
            throw new OptimisticLockException(
                    "Absence", command.id().toString(), command.version(), absence.getVersion());
        }
        ...
        validateCombinedDailyLimit(absence.getMemberId().value(), absence.getDate(), hours, command.id());
    ...
    private void validateCombinedDailyLimit(
            UUID memberId, java.time.LocalDate date, TimeAmount newAbsenceHours, UUID excludeAbsenceId) {
        // Get existing work hours for this date
        BigDecimal existingWorkHours = workLogRepository.getTotalHoursForDate(
                memberId, date, null // don't exclude any work entries
                );

        // Get existing absence hours for this date (excluding current if updating)
        BigDecimal existingAbsenceHours = absenceRepository.getTotalHoursForDate(memberId, date, excludeAbsenceId);

        // Calculate new combined total
        BigDecimal combinedTotal = existingWorkHours.add(existingAbsenceHours).add(newAbsenceHours.hours());
        ...
        if (combinedTotal.compareTo(MAX_DAILY_HOURS) > 0) {
            throw new DomainException(
                    ...
                    String.format(
                            ...
                            existingWorkHours, existingAbsenceHours, newAbsenceHours.hours(), combinedTotal));
        }
    }

...
public class AuthServiceImpl implements AuthService {
    ...
    public User signup(RegistrationRequest request) {
        ...
        if (userRepository.existsByEmail(normalizedEmail)) {
            // Tests expect IllegalArgumentException for duplicate email
        ...
        // Create user entity
        User user = User.create(normalizedEmail, request.name(), hashedPassword, defaultRole.getId());
    ...
    public LoginResponse login(LoginRequest request, String ipAddress, String userAgent) {
        ...
        if (!passwordEncoder.matches(request.password(), user.getHashedPassword())) {
            ...
            if (user.isLocked()) {
                auditLogService.logEvent(
                        ...
                        ipAddress,
                        "Account locked due to too many failed login attempts. Lock expires at: "
                                + user.getLockedUntil());
            }
        ...
        UserSession session = UserSession.create(
                UserId.of(user.getId().value()), ipAddress, userAgent, 30 // 30 minutes timeout
                );
        ...
        String rememberMeToken = request.rememberMe() ? generateRememberMeToken() : null;

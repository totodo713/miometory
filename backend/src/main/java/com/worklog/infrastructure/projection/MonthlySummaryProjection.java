...
public class MonthlySummaryProjection {
    ...
    private List<MonthlySummaryData.ProjectSummary> getProjectSummaries(
        ...
        for (Map<String, Object> row : results) {
            ...
            summaries.add(
                    new MonthlySummaryData.ProjectSummary(projectId.toString(), projectName, totalHours, percentage));
        }
     ...
     */
    private ApprovalStatusData getApprovalStatus(UUID memberId, int targetYear, int targetMonth) {
        // Calculate fiscal month date range (21st of previous month to 20th of target month)
        LocalDate fiscalMonthStart =
                LocalDate.of(targetYear, targetMonth, 1).minusMonths(1).withDayOfMonth(21);
        ...
        List<Map<String, Object>> results = jdbcTemplate.queryForList(sql, memberId, fiscalMonthStart, fiscalMonthEnd);

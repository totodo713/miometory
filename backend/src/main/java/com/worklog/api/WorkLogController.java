...
public class WorkLogController {
    ...
    public WorkLogController(WorkLogEntryService workLogService, JdbcWorkLogRepository workLogRepository) {
    ...
    public ResponseEntity<WorkLogEntryResponse> createEntry(@RequestBody CreateWorkLogEntryRequest request) {
        ...
        CreateWorkLogEntryCommand command = new CreateWorkLogEntryCommand(
                request.memberId(), request.projectId(), request.date(), request.hours(), request.comment(), enteredBy);
    ...
    public ResponseEntity<WorkLogEntriesResponse> getEntries(
        ...
        List<WorkLogEntry> entries = workLogRepository.findByDateRange(memberId, startDate, endDate, status);

        List<WorkLogEntryResponse> responses =
                entries.stream().map(this::toResponse).collect(Collectors.toList());

        WorkLogEntriesResponse response = new WorkLogEntriesResponse(responses, responses.size());
    ...
    public ResponseEntity<Void> updateEntry(
        ...
        UpdateWorkLogEntryCommand command =
                new UpdateWorkLogEntryCommand(id, request.hours(), request.comment(), updatedBy, version);

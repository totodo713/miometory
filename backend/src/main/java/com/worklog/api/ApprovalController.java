...
public class ApprovalController {
    ...
    public ResponseEntity<SubmitMonthResponse> submitMonth(@RequestBody SubmitMonthRequest request) {
        ...
        FiscalMonthPeriod fiscalMonth = new FiscalMonthPeriod(
            request.fiscalMonthStart(),
            request.fiscalMonthEnd()
        );

        SubmitMonthForApprovalCommand command = new SubmitMonthForApprovalCommand(
            MemberId.of(request.memberId()),
            fiscalMonth,
            MemberId.of(submittedBy)
        );

        MonthlyApprovalId approvalId = approvalService.submitMonth(command);

        SubmitMonthResponse response = new SubmitMonthResponse(approvalId.value().toString());
    ...
    public ResponseEntity<ApprovalQueueResponse> getApprovalQueue(
    ...
    public ResponseEntity<Void> approveMonth(
        @PathVariable UUID id,
        @RequestBody ApproveMonthRequest request
    ) {
        // Validate request
        ...
        ApproveMonthCommand command = new ApproveMonthCommand(
            MonthlyApprovalId.of(id),
            MemberId.of(request.reviewedBy())
        );
    ...
    public ResponseEntity<Void> rejectMonth(
        @PathVariable UUID id,
        @RequestBody RejectMonthRequest request
    ) {
        // Validate request
        if (request.reviewedBy() == null) {
            throw new DomainException("REVIEWED_BY_REQUIRED", "reviewedBy is required");

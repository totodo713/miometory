...
public record FiscalMonthPeriod(LocalDate startDate, LocalDate endDate) {
    ...
    public FiscalMonthPeriod {
        if (startDate == null || endDate == null) {
            throw new DomainException("FISCAL_PERIOD_NULL", "Start date and end date cannot be null");
        }

        if (!startDate.isBefore(endDate)) {
            throw new DomainException("FISCAL_PERIOD_INVALID_RANGE", "Start date must be before end date");
        }

        if (startDate.getDayOfMonth() != FISCAL_START_DAY) {
            throw new DomainException("FISCAL_PERIOD_INVALID_START", "Fiscal month must start on the 21st");
        }

        if (endDate.getDayOfMonth() != FISCAL_END_DAY) {
            throw new DomainException("FISCAL_PERIOD_INVALID_END", "Fiscal month must end on the 20th");
        }

        long daysBetween = ChronoUnit.DAYS.between(startDate, endDate);
        if (daysBetween < MIN_DAYS || daysBetween > MAX_DAYS) {
            throw new DomainException(
                    "FISCAL_PERIOD_INVALID_LENGTH",
                    String.format(
                            "Fiscal month must be approximately 1 calendar month (27-30 days), but was %d days",
                            daysBetween));
        }
    }

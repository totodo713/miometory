...
public record TimeAmount(BigDecimal hours) {
    ...
    public TimeAmount {
        ...
        if (hours.compareTo(MAX_HOURS) > 0) {
            throw new DomainException("TIME_AMOUNT_EXCEEDS_MAX", "Hours cannot exceed 24");
        }
        ...
        if (remainder.compareTo(ZERO) != 0) {
            throw new DomainException(
                    "TIME_AMOUNT_INVALID_INCREMENT", "Hours must be in 0.25 hour increments (15-minute intervals)");
        }
    }

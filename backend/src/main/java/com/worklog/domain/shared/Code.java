    ...
    public Code {
        ...
        if (value.length() < MIN_LENGTH || value.length() > MAX_LENGTH) {
            throw new DomainException("CODE_LENGTH", 
                String.format("Code must be between %d and %d characters", MIN_LENGTH, MAX_LENGTH));
        }
        if (!VALID_PATTERN.matcher(value).matches()) {
            throw new DomainException("CODE_FORMAT", 
                "Code can only contain alphanumeric characters and underscores");
        }
    }

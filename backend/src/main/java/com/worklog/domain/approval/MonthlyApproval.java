...
public class MonthlyApproval extends AggregateRoot<MonthlyApprovalId> {
    ...
    public static MonthlyApproval create(MemberId memberId, FiscalMonthPeriod fiscalMonth) {
        ...
        MonthlyApprovalCreated event = MonthlyApprovalCreated.create(
            approvalId,
            memberId,
            fiscalMonth
        );
    ...
    public void submit(
        ...
        Set<WorkLogEntryId> workLogEntryIds,
        Set<AbsenceId> absenceIds
    ) {
        if (!canSubmit()) {
        ...
        Set<UUID> workLogUuids = workLogEntryIds.stream()
            .map(WorkLogEntryId::value)
            .collect(Collectors.toSet());
        
        Set<UUID> absenceUuids = absenceIds.stream()
            .map(AbsenceId::value)
            .collect(Collectors.toSet());
        
        MonthSubmittedForApproval event = MonthSubmittedForApproval.create(
            this.id,
            submittedBy,
            workLogUuids,
            absenceUuids
    ...
    public void approve(MemberId reviewedBy) {
        if (!canApprove()) {
            throw new DomainException(
                "CANNOT_APPROVE_MONTH",
                "Cannot approve month in " + status + " status. Month must be SUBMITTED."
            );
        }
    ...
    public void reject(MemberId reviewedBy, String rejectionReason) {
        if (!canReject()) {
            throw new DomainException(
                "CANNOT_REJECT_MONTH",
                "Cannot reject month in " + status + " status. Month must be SUBMITTED."
            );
        }

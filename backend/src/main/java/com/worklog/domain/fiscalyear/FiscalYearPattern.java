...
public class FiscalYearPattern extends AggregateRoot<FiscalYearPatternId> {
    ...
    private static void validateStartMonth(int startMonth) {
        if (startMonth < 1 || startMonth > 12) {
            throw new DomainException(
                    "INVALID_START_MONTH", "Start month must be between 1 and 12, got: " + startMonth);
        }
    }

    private static void validateStartDay(int startDay) {
        if (startDay < 1 || startDay > 31) {
            throw new DomainException("INVALID_START_DAY", "Start day must be between 1 and 31, got: " + startDay);
        }
    }
    ...
    private static void validateDate(int startMonth, int startDay) {
        try {
            ...
            throw new DomainException(
                    "INVALID_DATE", String.format("Invalid date combination: month=%d, day=%d", startMonth, startDay));
        }
    }

...
public record AbsenceStatusChanged(
    ...
    public static AbsenceStatusChanged create(AbsenceId id, String fromStatus, String toStatus, MemberId changedBy) {
        return new AbsenceStatusChanged(
                UUID.randomUUID(), Instant.now(), id.value(), fromStatus, toStatus, changedBy.value());
    }

...
public class Organization extends AggregateRoot<OrganizationId> {
    ...
    public void assignPatterns(UUID fiscalYearPatternId, UUID monthlyPeriodPatternId) {
        OrganizationPatternAssigned event = OrganizationPatternAssigned.create(
                this.id.value(),
                fiscalYearPatternId,
    ...
    private static void validateLevel(int level) {
        if (level < 1 || level > MAX_HIERARCHY_LEVEL) {
            throw new DomainException(
                    "INVALID_LEVEL",
                    "Organization level must be between 1 and " + MAX_HIERARCHY_LEVEL
            );
        }
    }
    ...
    private static void validateParentConsistency(OrganizationId parentId, int level) {
        if (level == 1 && parentId != null) {
            throw new DomainException(
                    "ROOT_CANNOT_HAVE_PARENT",
                    "Root organization (level 1) cannot have a parent"
            );
        }
        if (level > 1 && parentId == null) {
            throw new DomainException(
                    "NON_ROOT_MUST_HAVE_PARENT",
                    "Non-root organization must have a parent"
            );
        }
    }
